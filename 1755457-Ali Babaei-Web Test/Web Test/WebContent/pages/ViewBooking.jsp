...
<div class="middle_div">
	<form name="search"
		...
		<input type="hidden" id="page" name="page" value="view_page">
		<br> <label for="date">Enter Date: </label><br> <input
			type="text" name="booking_date" placeholder="DD/MM/YYYY"> <br>
		...
		<table name="search-results" border="1" align="center">
			...
			<%
								List<Booking> bookingList =  (List<Booking>) request.getAttribute("booking_search");
								if(bookingList != null) {
									for(Booking booking: bookingList) {
							%>
			<tr>
				<td><%= booking.getCustomername()%></td>
				<td><%= booking.getLicenseNo()%></td>
				<td><%= booking.getVehicleMake()%></td>
	...
	<form action="<%=request.getContextPath()%>/pages/Homepage.jsp">
